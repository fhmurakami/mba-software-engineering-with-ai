...
export class ChannelOwnerGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest();
    return this.channelsService.isOwner(
...
- **Guards, Interceptors, Pipes, Filters** handle infrasturcture concerns only; when they need a business decision, they inject a Service
